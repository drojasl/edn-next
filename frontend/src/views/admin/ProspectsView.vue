...
const exportCSV = () => {
    ...
    link.setAttribute('download', `${t('admin.prospects.csv.filename')}_${new Date().toISOString().split('T')[0]}.csv`)
...
<template>
    <div @click="activeMenuId = null">
        ...
        >
            <template #actions>
                ...
                >
                    ...
                    <span class="hidden sm:block">{{ $t('admin.prospects.export') }}</span>
                </button>
            </template>
        </AdminPageHeader>
        ...
        <AdminDataTable
            ...
            :emptyMessage="$t('admin.prospects.table.no_prospects')"
        >
            ...
            >
                ...
                <td class="px-6 py-4 text-slate-500 whitespace-nowrap" :class="{'font-bold text-slate-900': !prospect.is_reviewed}">{{ prospect.email }}</td>
                <td class="px-6 py-4 text-slate-500 whitespace-nowrap" :class="{'font-bold text-slate-900': !prospect.is_reviewed}">{{ prospect.phone || '-' }}</td>
                <td class="px-6 py-4 text-slate-500 whitespace-nowrap">
                    ...
                    <span v-else class="text-slate-400 italic">{{ $t('admin.prospects.table.unspecified') }}</span>
                </td>
                <td class="px-6 py-4 text-slate-400 text-xs font-medium whitespace-nowrap">{{ formatDate(prospect.created_at) }}</td>
                <td class="px-6 py-4 text-right relative whitespace-nowrap">
                    ...
                    <div v-if="activeMenuId === prospect.id" class="absolute right-6 top-10 w-48 bg-white rounded-xl shadow-2xl border border-slate-100 z-[100] py-2">
                        ...
                        >
                            ...
                            {{ prospect.is_reviewed ? $t('admin.prospects.table.mark_new') : $t('admin.prospects.table.mark_reviewed') }}
                        </button>
                    </div>
                </td>
            </tr>
