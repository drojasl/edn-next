...
const nodeExtent = [
  [EDITOR_CONFIG.bounds.minX, EDITOR_CONFIG.bounds.minY],
  [EDITOR_CONFIG.bounds.maxX, EDITOR_CONFIG.bounds.maxY],
      ...
      if (node.options && node.options.length) {
        node.options.forEach((opt: any) => {
          if (opt.next_node_id) {
            ...
            if (node.type === 'menu') {
              edge.sourceHandle = opt.label?.startsWith('menu-btn-')
                ? opt.label
                : opt.label // Keep opt.label for fallback legacy string connections
            }
...
const handleNodeSave = async (formData: NodeData) => {
  ...
  if (response.success) {
    await fetchNodes()
    showToast(t('course.editor.save_success'), 'success')
  } else {
    showToast(response.error?.message || t('common.error'), 'error')
  }
  isSaving.value = false
...
const handleAction = async ({ type, id }: { type: string; id: string }) => {
  if (type === 'edit') {
    ...
    const hasConnections = initialEdges.value.some(
...
<template>
  <div class="h-screen flex flex-col">
    <header
      class="bg-white border-b border-slate-200 p-3 sm:p-4 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 sm:gap-4 z-10"
    >
      <div class="flex flex-wrap items-center gap-y-1 gap-x-2 text-slate-600">
        ...
        <div class="flex items-center gap-1 sm:gap-2 text-sm sm:text-base">
          <router-link
            ...
            >{{ $t('course.editor.breadcrumbs.courses') }}</router-link
          >
          ...
          >
            ...
            <svg
              ...
              stroke-width="2"
              stroke="currentColor"
              class="w-3.5 h-3.5 opacity-0 group-hover:opacity-50 transition-opacity"
            >
              <path
