...
<template>
  <div class="p-8 max-w-7xl mx-auto">
    ...
    >
      <tr v-for="user in users" :key="user.id" class="hover:bg-slate-50 transition-colors border-b border-slate-100 last:border-0">
        <td class="px-6 py-4">
          <div class="flex flex-col">
            <span class="font-bold text-slate-900">{{ user.name }} {{ user.last_name }}</span>
            <span class="text-xs text-slate-500 font-medium tracking-tight">slug: {{ user.slug }}</span>
          </div>
        </td>
        <td class="px-6 py-4">
          <div class="flex flex-col">
            <span class="text-slate-700 font-medium">{{ user.codigo_amway }}</span>
            <span class="text-xs text-slate-500">{{ user.is_account_holder ? $t('admin.users.role.holder') : $t('admin.users.role.coholder') }}</span>
          </div>
        </td>
        <td class="px-6 py-4 text-slate-700 whitespace-nowrap">{{ user.email }}</td>
        <td class="px-6 py-4 whitespace-nowrap">
          <span 
            class="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-bold tracking-wide"
            :class="user.is_active ? 'bg-emerald-50 text-emerald-700 ring-1 ring-inset ring-emerald-600/20' : 'bg-rose-50 text-rose-700 ring-1 ring-inset ring-rose-600/20'"
          >
