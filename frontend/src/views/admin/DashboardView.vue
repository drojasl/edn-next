...
const globalStats = computed(() => {
    ...
    const convRate = totalVisits > 0 ? (totalRegistered / totalVisits * 100).toFixed(1) : '0';

    return [
        { name: t('admin.dashboard.stats.total_visits'), value: totalVisits.toString(), change: '', changeType: 'neutral' },
        { name: t('admin.dashboard.stats.total_prospects'), value: totalRegistered.toString(), change: '', changeType: 'neutral' },
        { name: t('admin.dashboard.stats.active_codes'), value: funnelData.value.length.toString(), change: '', changeType: 'neutral' },
        { name: t('admin.dashboard.stats.conversion_rate'), value: `${convRate}%`, change: '', changeType: 'neutral' },
    ];
});
</script>
    ...
    <div class="space-y-8">
        ...
        <div class="space-y-6">
            <div class="flex items-center justify-between">
                <h2 class="text-xl font-bold text-slate-800">{{ $t('admin.dashboard.funnel_title') || 'Rendimiento por Código' }}</h2>
                <button @click="fetchStats" class="text-indigo-600 hover:text-indigo-700 font-medium text-sm flex items-center gap-2">
            ...
            <div v-if="loading && funnelData.length === 0" class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                <div v-for="i in 2" :key="i" class="bg-white h-96 rounded-2xl border border-slate-100 animate-pulse"></div>
            </div>

            <div v-else-if="funnelData.length === 0" class="bg-white rounded-2xl border border-slate-100 p-12 text-center text-slate-500">
                ...
                >
                        ...
                        <div class="flex items-center gap-4 ml-auto">
                            ...
                            <div class="hidden md:flex gap-8 border-l border-slate-200 pl-8 mr-4">
                                <div class="text-center">
                                    ...
                                    <div class="text-[10px] font-bold text-slate-400 uppercase tracking-tighter">Visitas</div>
                                </div>
                                <div class="text-center">
                                    <div class="text-xl font-black text-emerald-600">{{ codeStat.registered }}</div>
                    ...
                    >
                        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-x-12 gap-y-8">
                            <div v-for="(node, idx) in codeStat.funnel" :key="node.id" class="space-y-2">
                                <div class="flex justify-between text-sm font-bold">
                                    <span class="text-slate-700 truncate pr-2">{{ Number(idx) + 1 }}. {{ node.title }}</span>
                                    <span class="text-slate-900 shrink-0">{{ node.views }} <span class="text-slate-400 font-medium">({{ codeStat.visits > 0 ? (node.views/codeStat.visits*100).toFixed(0) : 0 }}%)</span></span>
                                </div>
