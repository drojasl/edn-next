...
const addSocialLink = () => {
  ...
  if (/(http|https|www\.|\.com|\.net|\.org)/i.test(currentValue.value)) {
    currentSocialError.value = 'Por favor, usa solo el usuario (@nombre) o teléfono, no la URL completa.'
    return
  }
...
<template>
  <form @submit.prevent="handleSubmit" class="space-y-6">
    ...
    <div v-if="mode !== 'register'" class="bg-slate-50 p-6 rounded-xl border border-slate-100 space-y-4">
      <h4 class="text-sm font-bold text-slate-800 border-b border-slate-200 pb-2">Datos de Contacto y Redes</h4>
         ...
         <div class="relative w-full md:w-1/3">
            ...
            >
              <div class="flex items-center gap-2">
                 <svg v-if="currentPlatform === 'cell_phone'" class="w-5 h-5 flex-shrink-0" :class="socialPlatforms.find(p => p.id === currentPlatform)?.color" fill="none" stroke="currentColor" viewBox="0 0 24 24" v-html="socialPlatforms.find(p => p.id === currentPlatform)?.icon"></svg>
                 <svg v-else class="w-5 h-5 flex-shrink-0" :class="socialPlatforms.find(p => p.id === currentPlatform)?.color" fill="currentColor" viewBox="0 0 24 24" v-html="socialPlatforms.find(p => p.id === currentPlatform)?.icon"></svg>
                 <span class="truncate max-w-[100px] md:max-w-[70px] lg:max-w-[120px] text-left">{{ socialPlatforms.find(p => p.id === currentPlatform)?.name }}</span>
              </div>
         ...
         <div class="w-full flex-1">
           <div class="flex gap-2">
             <input
                ...
                type="text"
                :placeholder="socialPlatforms.find(p => p.id === currentPlatform)?.placeholder"
                class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition-all"
