...
const socialPlatforms = computed(() => [
  {
    id: 'cell_phone',
    name: t('admin.users.social.platforms.cell_phone.label'),
    placeholder: t('admin.users.social.platforms.cell_phone.placeholder'),
    color: 'text-slate-600',
    icon: 'M12 18h.01M8 21h8a2 2 0 002-2V5a2 2 0 00-2-2H8a2 2 0 00-2 2v14a2 2 0 002 2z',
  },
  {
    id: 'whatsapp',
    name: t('admin.users.social.platforms.whatsapp.label'),
    placeholder: t('admin.users.social.platforms.whatsapp.placeholder'),
  ...
  {
    id: 'telegram',
    name: t('admin.users.social.platforms.telegram.label'),
    placeholder: t('admin.users.social.platforms.telegram.placeholder'),
    color: 'text-sky-500',
  ...
  {
    id: 'instagram',
    name: t('admin.users.social.platforms.instagram.label'),
    placeholder: t('admin.users.social.platforms.instagram.placeholder'),
    color: 'text-pink-600',
  ...
  {
    id: 'facebook',
    name: t('admin.users.social.platforms.facebook.label'),
    placeholder: t('admin.users.social.platforms.facebook.placeholder'),
    color: 'text-blue-600',
  ...
  {
    id: 'tiktok',
    name: t('admin.users.social.platforms.tiktok.label'),
    placeholder: t('admin.users.social.platforms.tiktok.placeholder'),
  ...
  {
    id: 'youtube',
    name: t('admin.users.social.platforms.youtube.label'),
    placeholder: t('admin.users.social.platforms.youtube.placeholder'),
    color: 'text-red-600',
    icon: 'M23.498 6.186a3.016 3.016 0 0 0-2.122-2.136C19.505 3.5 12 3.5 12 3.5s-7.505 0-9.377.55a3.016 3.016 0 0 0-2.122 2.136C0 8.07 0 12 0 12s0 3.93.501 5.814a3.016 3.016 0 0 0 2.122 2.136c1.871.55 9.377.55 9.377.55s7.505 0 9.377-.55a3.016 3.016 0 0 0 2.122-2.136C24 15.93 24 12 24 12s0-3.93-.502-5.814zM9.545 15.568V8.432L15.818 12l-6.273 3.568z',
  },
])
...
watch(
  [() => form.value.name, () => form.value.last_name],
  ([name, lastName]) => {
...
<template>
  <form class="space-y-6" @submit.prevent="handleSubmit">
    ...
    <!-- Amway Links personal links (Only for Profile or Edit) -->
    <div
      v-if="mode !== 'register'"
    ...
    >
      <div>
        <label
          ...
          class="block text-sm font-medium text-gray-700 mb-2"
        >
          {{ t('profile.abo_link_label') }}
        </label>
        <input
          id="abo_link"
          v-model="form.abo_link"
          type="url"
          :placeholder="t('profile.abo_link_placeholder')"
          class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition-all"
          :disabled="loading"
        />
      </div>
      ...
      <div>
        <label
          ...
          class="block text-sm font-medium text-gray-700 mb-2"
        >
          {{ t('profile.client_link_label') }}
        </label>
        <input
          id="client_link"
          v-model="form.client_link"
          type="url"
          :placeholder="t('profile.client_link_placeholder')"
          class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition-all"
          :disabled="loading"
        />
      </div>
    ...
    <!-- Digital Store Link (Only for Profile or Edit) -->
    <div v-if="mode !== 'register'">
      <label
        for="my_digital_store"
        class="block text-sm font-medium text-gray-700 mb-2"
      >
        {{ t('profile.my_digital_store_label') }}
      </label>
      <input
        id="my_digital_store"
        v-model="form.my_digital_store"
        type="url"
        :placeholder="t('profile.my_digital_store_placeholder')"
        class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition-all"
        :disabled="loading"
      />
    </div>
    ...
    <div
      v-if="mode !== 'register'"
      class="bg-slate-50 p-6 rounded-xl border border-slate-100 space-y-4"
    >
      <h4
        class="text-sm font-bold text-slate-800 border-b border-slate-200 pb-2"
      >
        {{ $t('admin.users.social.title') }}
      </h4>
      ...
      <div class="flex flex-col md:flex-row gap-3 items-start">
        <div class="relative w-full md:w-1/3">
          <button
            ...
            :class="{
              'ring-2 ring-indigo-500 border-transparent': showPlatformDropdown,
            }"
          ...
          >
            <div class="flex items-center gap-2">
              <svg
                v-if="currentPlatform === 'cell_phone'"
                class="w-5 h-5 flex-shrink-0"
                :class="getPlatformInfo(currentPlatform)?.color"
                fill="none"
                stroke="currentColor"
                viewBox="0 0 24 24"
              >
                <path
                  ...
                  stroke-width="2"
                  :d="getPlatformInfo(currentPlatform)?.icon"
                />
              </svg>
              <svg
                v-else
                ...
                viewBox="0 0 24 24"
              >
                <path :d="getPlatformInfo(currentPlatform)?.icon" />
              </svg>
              <span
                class="truncate max-w-[100px] md:max-w-[70px] lg:max-w-[120px] text-left"
                >{{ getPlatformInfo(currentPlatform)?.name }}</span
              >
            </div>
            <svg
          ...
          >
            <button
              ...
              @mousedown.prevent="selectPlatform(platform.id)"
            >
              <svg
                v-if="platform.id === 'cell_phone'"
                class="w-5 h-5 flex-shrink-0"
        ...
        <div class="w-full flex-1">
          <div class="flex gap-2">
            ...
            <button
              type="button"
              class="bg-green-600 text-white px-4 py-3 rounded-lg hover:bg-green-700 transition flex-shrink-0"
      ...
      <div v-if="form.social_links.length > 0" class="space-y-2 mb-4">
        <div
          ...
          :key="index"
          class="flex items-center justify-between bg-white px-4 py-2 rounded-lg shadow-sm border border-slate-200"
        >
          <div class="flex items-center space-x-3">
            <div class="w-6 h-6 flex items-center justify-center">
              <svg
                v-if="link.platform === 'cell_phone'"
                class="w-5 h-5"
                :class="
                  socialPlatforms.find((p) => p.id === 'cell_phone')?.color
                "
                fill="none"
                stroke="currentColor"
                viewBox="0 0 24 24"
              >
                <path
                  ...
                  stroke-linejoin="round"
                  stroke-width="2"
                  :d="socialPlatforms.find((p) => p.id === 'cell_phone')?.icon"
                />
              </svg>
              <svg
                v-else
                class="w-5 h-5"
                :class="
                  socialPlatforms.find((p) => p.id === link.platform)?.color
                "
                fill="currentColor"
                viewBox="0 0 24 24"
              >
                <path
                  :d="socialPlatforms.find((p) => p.id === link.platform)?.icon"
                />
              </svg>
            </div>
    ...
    <div v-if="!isProfile" class="grid grid-cols-1 md:grid-cols-2 gap-6">
      <div>
        ...
        >
          ...
          <span v-if="mode === 'edit'" class="text-xs text-gray-500 font-normal"
