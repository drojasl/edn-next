...
watch(
  () => [props.initialNodes, props.initialEdges] as const,
  async (newVal) => {
    ...
    if (structuralChange) {
      ...
      const simplifiedOldEdges = edges.value.map(
        (e) => `${e.source}-${e.sourceHandle}-${e.target}-${e.label}`
      )
      const simplifiedNewEdges = newEdges.map(
        (e) => `${e.source}-${e.sourceHandle}-${e.target}-${e.label}`
      )

      if (
        JSON.stringify(simplifiedOldEdges) !==
        JSON.stringify(simplifiedNewEdges)
      ) {
        edges.value = JSON.parse(JSON.stringify(newEdges))
      }
    }
...
onConnect(async (params: Connection) => {
  ...
  let displayLabel =
    params.sourceHandle || t('course.editor.default_connection_label')
...
const isValidConnection = (connection: Connection) => {
  ...
  if (connection.sourceHandle) {
    const handleHasAnotherEdge = edges.value.some(
      (edge) =>
  ...
  const sourceHasAnotherEdge = edges.value.some(
    (edge) => edge.source === connection.source && !edge.sourceHandle // Normal nodes have no specific sourceHandle
  )
...
<template>
  <div class="h-full w-full relative">
    ...
    >
      <div
        class="bg-white/80 p-3 rounded-full shadow-lg border border-slate-200"
      >
        <div
    ...
    >
      ...
      <template #edge-custom="edgeProps">
        <ConnectionEdge
          v-bind="edgeProps"
          @label-change="
            (label: string) => handleLabelChange(edgeProps.id, label)
          "
        />
      </template>
    </VueFlow>
